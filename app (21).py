...
def load_data():
    try:
        ...
        h = pd.read_csv(f"{GITHUB_RAW}/hitters_today.csv?cb={bust}")
        p = pd.read_csv(f"{GITHUB_RAW}/pitchers_today.csv?cb={bust}")
        m = pd.read_csv(f"{GITHUB_RAW}/matchups_today.csv?cb={bust}")
        return h, p, m
...
def hr_score_100(barrel, pulled_brl, fb, iso, xslg, hh, la, ev, p_barrel, p_fb, p_hr9, bats, throws, form, bbe):
    """
    HR Score (0-100)
    Barrel%       × 2.2  = max 33  (15% barrel = elite)
    PulledBrl%    × 0.5  = max 12.5
    FB%           × 0.2  = max 7
    ISO           × 40   = max 8
    HardHit%      × 0.12 = max 6
    EV bonus      >93 = +6, >91 = +3
    LA bonus      sweet spot 10-25° = +4
    P_Barrel      × 0.4  = max 6
    Platoon       × 1.05-1.10 multiplier
    Form          adjustment ±4
    """
    if bbe < 20: return None  # small sample
    raw = (barrel * 1.8) + (pulled_brl * 0.45) + (fb * 0.18) + (iso * 35) + (hh * 0.10) + (p_barrel * 0.35)
    if ev >= 93: raw += 5
    elif ev >= 91: raw += 2.5
    if 10 <= la <= 25: raw += 3.5
...
def tb_score_100(xslg, hh, ld, sweet, ev, iso, pull, fb, p_ld, form, bats, throws):
    """
    Total Bases Score (0-100)
    xSLG          × 60   = max ~33  (core TB predictor)
    HardHit%      × 0.25 = max ~12
    LD%           × 0.35 = max ~12  (line drives = singles/doubles)
    SweetSpot%    × 0.25 = max ~9
    EV bonus      >93 = +6, >91 = +3
    ISO           × 25   = max ~6   (extra base power)
    Pull%         × 0.10 = max ~5   (pull hitters drive more XBH)
    P_LD%         × 0.20 = max ~6   (pitcher allows line drives)
    Platoon       × 1.05-1.10
    Form          ±3
    """
    raw = (xslg * 60) + (hh * 0.25) + (ld * 0.35) + (sweet * 0.25) + (iso * 25) + (pull * 0.10) + (p_ld * 0.20)
    if ev >= 93: raw += 6
    elif ev >= 91: raw += 3
    raw *= platoon_bonus(bats, throws)
    try:
...
def build_player_data():
    ...
    for _, m in matchups_df.iterrows():
        ...
        sweet=g(h,"SweetSpot%"); oppo=g(h,"Oppo%"); pull=g(h,"PulledBrl%"); pull=g(h,"FB%")
        ...
        xslg_val = xwoba * 1.4
        tb_s = tb_score_100(xslg_val,hh,ld,sweet,ev,iso,pulledbrl,fb,p_ld,form,bats,throws)
        ...
        rows.append({
            ...
            "hr_score": hr_s, "hit_score": hit_s, "tb_score": tb_s,
...
def render_card(row, score_key, score_label, reasons_key, rank):
    ...
    elif score_key == "tb_score":
        cells = [
            ("xSLG", f"{row['xwoba']*1.4:.3f}", heat_class(row['xwoba']*1.4,[0.50,0.40,0.32])),
            ("HH%", f"{row['hh']:.1f}%", heat_class(row['hh'],[50,40,30])),
            ("LD%", f"{row['ld']:.1f}%", heat_class(row['ld'],[25,20,15])),
            ("EV", f"{row['ev']:.0f}", heat_class(row['ev'],[93,90,87])),
            ("Sweet%", f"{row['sweet']:.1f}%", heat_class(row['sweet'],[35,28,20])),
...
# ── TABS ───────────────────────────────────────────────────────────────────────
tab1, tab2, tab3, tab4, tab5 = st.tabs(["🚀 HR Targets", "🎯 Hit Targets", "⚡ XBH Targets", "📐 Double Targets", "💥 Total Bases"])
...
def filtered_sorted(players, score_key, top_n=15):
...
def render_tab(players, score_key, score_label, reasons_key, title, top_n=15):
    top = filtered_sorted(players, score_key, top_n)
    if not top:
        st.info("No data loaded yet.")
        return
    cards_html = ""
    for i, r in enumerate(top):
        cards_html += render_card(r, score_key, score_label, reasons_key, i+1)
    
    # Get CSS from the page style
    full_html = f"""
    <html><head>
    ...
    :root {{
        ...
        --text: #e8e8f5; --muted: #6666aa;
        --green: #00ff88; --yellow: #ffcc00;
        --orange: #ff8800; --red: #ff3355;
    }}
    * {{ font-family: 'Space Grotesk', sans-serif; box-sizing: border-box; margin: 0; padding: 0; }}
    body {{ background: var(--bg); color: var(--text); padding: 8px; }}
    .section-label {{
        font-size: 0.65rem; font-weight: 700; text-transform: uppercase;
        letter-spacing: 3px; color: var(--muted); margin-bottom: 1rem;
    ...
    .player-card {{
        ...
        border-radius: 10px; padding: 1rem 1.2rem; position: relative;
    }}
    .card-rank {{ position: absolute; top: 10px; right: 12px; font-size: 0.65rem; font-family: 'Space Mono', monospace; color: var(--muted); }}
    .card-rank.top1 {{ color: var(--green); font-weight: 700; }}
    .card-rank.top3 {{ color: var(--yellow); }}
    .player-name {{ font-size: 1rem; font-weight: 600; margin-bottom: 2px; }}
    .player-meta {{ font-size: 0.72rem; color: var(--muted); margin-bottom: 10px; }}
    ...
    .platoon-badge {{
        ...
        font-weight: 600; letter-spacing: 1px; margin-left: 6px;
    }}
    .form-badge {{ display: inline-block; font-size: 0.65rem; padding: 1px 6px; border-radius: 3px; margin-left: 4px; }}
    .form-hot {{ background: rgba(255,136,0,0.15); color: var(--orange); border: 1px solid rgba(255,136,0,0.3); }}
    .form-up {{ background: rgba(0,255,136,0.1); color: var(--green); border: 1px solid rgba(0,255,136,0.2); }}
    .form-neutral {{ background: rgba(102,102,170,0.15); color: var(--muted); border: 1px solid var(--border); }}
    .form-down {{ background: rgba(255,51,85,0.1); color: var(--red); border: 1px solid rgba(255,51,85,0.2); }}
    .small-sample {{ background: rgba(255,136,0,0.1); color: var(--orange); border: 1px solid rgba(255,136,0,0.3); font-size: 0.6rem; padding: 1px 6px; border-radius: 3px; margin-left: 6px; }}
    .heat-grid {{ display: flex; gap: 6px; flex-wrap: wrap; margin-top: 8px; }}
    .heat-cell {{ padding: 3px 8px; border-radius: 4px; font-size: 0.68rem; font-family: 'Space Mono', monospace; display: flex; flex-direction: column; align-items: center; min-width: 55px; }}
    .heat-label {{ font-size: 0.55rem; text-transform: uppercase; letter-spacing: 0.5px; opacity: 0.7; margin-bottom: 1px; }}
    .heat-val {{ font-weight: 700; }}
    .heat-green {{ background: rgba(0,255,136,0.12); color: var(--green); }}
    .heat-yellow {{ background: rgba(255,204,0,0.12); color: var(--yellow); }}
    .heat-orange {{ background: rgba(255,136,0,0.12); color: var(--orange); }}
    .heat-red {{ background: rgba(255,51,85,0.12); color: var(--red); }}
    .heat-neutral {{ background: #161625; color: var(--muted); }}
    </style></head><body>
    <div class="section-label">{title}</div>
    <div class="cards-grid">{cards_html}</div>
    </body></html>
    """
    components.html(full_html, height=len(top) * 320, scrolling=True)

with tab1:
    render_tab(all_players, "hr_score", "HR Score", "hr_reasons", "Top 20 HR Candidates Today", 20)

with tab2:
    render_tab(all_players, "hit_score", "Hit Score", "hit_reasons", "Top 20 Hit Candidates Today", 20)

with tab3:
    render_tab(all_players, "xbh_score", "XBH Score", "hr_reasons", "Top 20 XBH Candidates Today", 20)

with tab4:
    render_tab(all_players, "dbl_score", "Double Score", "hit_reasons", "Top 20 Double Candidates Today", 20)

with tab5:
    render_tab(all_players, "tb_score", "Total Bases Score", "hr_reasons", "Top 20 Total Bases Candidates Today", 20)

# ── RESULTS TRACKER ───────────────────────────────────────────────────────────
st.markdown("---")
with st.expander("📊 Results Tracker — How is the model performing?"):
    try:
        results_url = f"{GITHUB_RAW}/results_log.csv"
        results_df = pd.read_csv(results_url)
        
        total = len(results_df[results_df['HR_Score'] > 0])
        hits = len(results_df[(results_df['HR_Score'] > 0) & (results_df['HR'] == 1)])
        hit_rate = (hits/total*100) if total > 0 else 0
        
        top10_df = results_df[results_df['HR_Rank'] <= 10]
        top10_hits = len(top10_df[top10_df['HR'] == 1])
        top10_total = len(top10_df[top10_df['HR_Rank'] > 0])
        top10_rate = (top10_hits/top10_total*100) if top10_total > 0 else 0
        
        kasper_df = results_df[results_df['Kasper_Pick'] == 'Yes']
        kasper_hits = len(kasper_df[kasper_df['HR'] == 1])
        kasper_total = len(kasper_df)
        kasper_rate = (kasper_hits/kasper_total*100) if kasper_total > 0 else 0
        
        overlap_df = results_df[(results_df['HR_Rank'] <= 10) & (results_df['Kasper_Pick'] == 'Yes')]
        overlap_hits = len(overlap_df[overlap_df['HR'] == 1])
        overlap_total = len(overlap_df)
        overlap_rate = (overlap_hits/overlap_total*100) if overlap_total > 0 else 0
        
        stats_html = f"""
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin:12px 0">
            <div style="background:#0f0f1a;border:1px solid #1e1e35;border-radius:8px;padding:12px;text-align:center">
                <div style="font-size:0.65rem;color:#6666aa;text-transform:uppercase;letter-spacing:2px">Board Top 10 HR Rate</div>
                <div style="font-size:1.8rem;font-weight:700;color:#00ff88;font-family:monospace">{top10_rate:.1f}%</div>
                <div style="font-size:0.7rem;color:#6666aa">{top10_hits}/{top10_total} hit</div>
            </div>
            <div style="background:#0f0f1a;border:1px solid #1e1e35;border-radius:8px;padding:12px;text-align:center">
                <div style="font-size:0.65rem;color:#6666aa;text-transform:uppercase;letter-spacing:2px">Kasper Pick Rate</div>
                <div style="font-size:1.8rem;font-weight:700;color:#ffcc00;font-family:monospace">{kasper_rate:.1f}%</div>
                <div style="font-size:0.7rem;color:#6666aa">{kasper_hits}/{kasper_total} hit</div>
            </div>
            <div style="background:#0f0f1a;border:1px solid #1e1e35;border-radius:8px;padding:12px;text-align:center">
                <div style="font-size:0.65rem;color:#6666aa;text-transform:uppercase;letter-spacing:2px">Board + Kasper Overlap</div>
                <div style="font-size:1.8rem;font-weight:700;color:#aa44ff;font-family:monospace">{overlap_rate:.1f}%</div>
                <div style="font-size:0.7rem;color:#6666aa">{overlap_hits}/{overlap_total} hit</div>
            </div>
            <div style="background:#0f0f1a;border:1px solid #1e1e35;border-radius:8px;padding:12px;text-align:center">
                <div style="font-size:0.65rem;color:#6666aa;text-transform:uppercase;letter-spacing:2px">Days Tracked</div>
                <div style="font-size:1.8rem;font-weight:700;color:#4488ff;font-family:monospace">{results_df['Date'].nunique()}</div>
                <div style="font-size:0.7rem;color:#6666aa">days of data</div>
            </div>
        </div>
        """
        ...
        display_df = results_df[['Date','Player','Team','HR','HR_Score','HR_Rank','Kasper_Pick']].copy()
        display_df['Result'] = display_df['HR'].apply(lambda x: '✅ HR' if x == 1 else '❌ No HR')
        st.dataframe(display_df[['Date','Player','Team','Result','HR_Score','HR_Rank','Kasper_Pick']], use_container_width=True)
        
    except Exception as e:
        st.info("No results data yet. Upload results_log.csv to GitHub to start tracking!")
