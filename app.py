...
with tab1:
    ...
    if top:
        st.markdown('<div class="section-label">Top 10 HR Candidates Today</div>', unsafe_allow_html=True)
        st.markdown('<div class="cards-grid">' + "".join([render_card(r, "hr_score", "HR Score", "hr_reasons", i+1) for i,r in enumerate(top)]) + '</div>', unsafe_allow_html=True)
    else:
...
with tab2:
    ...
    if top:
        st.markdown('<div class="section-label">Top 10 Hit Candidates Today</div>', unsafe_allow_html=True)
        st.markdown('<div class="cards-grid">' + "".join([render_card(r, "hit_score", "Hit Score", "hit_reasons", i+1) for i,r in enumerate(top)]) + '</div>', unsafe_allow_html=True)

with tab3:
    top = filtered_sorted(all_players, "xbh_score", 10)
    if top:
        st.markdown('<div class="section-label">Top 10 Extra Base Hit Candidates Today</div>', unsafe_allow_html=True)
        st.markdown('<div class="cards-grid">' + "".join([render_card(r, "xbh_score", "XBH Score", "hr_reasons", i+1) for i,r in enumerate(top)]) + '</div>', unsafe_allow_html=True)

with tab4:
    top = filtered_sorted(all_players, "dbl_score", 10)
    if top:
        st.markdown('<div class="section-label">Top 10 Double Candidates Today</div>', unsafe_allow_html=True)
        st.markdown('<div class="cards-grid">' + "".join([render_card(r, "dbl_score", "Double Score", "hit_reasons", i+1) for i,r in enumerate(top)]) + '</div>', unsafe_allow_html=True)
