...
HITTERS_URL = f"{GITHUB_RAW}/hitters%20(2).csv"
PITCHERS_URL = f"{GITHUB_RAW}/pitchers.csv"
MATCHUPS_URL = f"{GITHUB_RAW}/matchups%20(1).csv"
...
# ── MAIN TABS ──────────────────────────────────────────────────────────────────
tab1, tab2 = st.tabs(["📊 Daily Board", "⚾ Pitchers"])
...
with tab1:
    if hitters_df.empty or matchups_df.empty:
        ...
        <div style="text-align:center; padding: 80px 0; color: #555570;">
            ...
            <div style="font-size: 0.85rem;">If this persists, check your GitHub CSV files</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        ...
        if not rows:
            st.warning("No matching data found. Check player names match between CSVs!")
        else:
