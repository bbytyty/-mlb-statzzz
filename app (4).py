...
def doubles_likelihood(score):
    if score >= 2200: return "🔥 Double Lock"
    if score >= 1800: return "🟢 Double Play"
    if score >= 1400: return "🟡 Moderate"
    return "❌ Low"
...
def doubles_play_fade(doubles_score, form):
    try: f = float(form)
    except: f = 5
    if doubles_score >= 2200 and f >= 6: return "🔥 Double Lock"
    if doubles_score >= 1800 and f >= 5: return "🟢 Double Play"
    if doubles_score >= 1400: return "🟡 Double Lean"
    return "❌ Skip"

def hr_play_fade(master, barrel, p_csw, form, bats, throws):
    try: f = float(form)
    except: f = 5
    platoon = platoon_bonus(bats, throws)
    if barrel < 5: return "❌ Fade HR"
...
def build_rows(filtered, h_idx, p_idx):
    ...
    for _, m in filtered.iterrows():
        ...
        hr_pf=hr_play_fade(master_s,barrel,p_csw,form,bats,throws)
        hit_pf=hit_play_fade(hit_s,form)
        dbl_pf=doubles_play_fade(dbl_s,form)
